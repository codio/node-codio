...
ProjectManager.prototype.getProjectForListener = function (name, info) {
    ...
    return this.requestSigned('getProjectForShare', {
        accountName : info.user,
        projectName: name
    }, {
        'session_id' : info.session
...
// Get a project archived state by name without session.
//
// name     - String, the project name.
// info     - Object, with additional parameters
//            user    - String, the user name.
//
// Returns a promise.
ProjectManager.prototype.getProjectArchivedState = function (name, info) {
    assert.string(name, 'name');
    assert.object(info, 'info');
    assert.string(info.user, 'info.user');

    return this.requestSigned('getProjectArchivedState', {
        accountName : info.user,
        projectName: name
    }, {
        'session_id' : info.session
...
// Get all free private projects of the currently signed in user.
//
// session - String, the session key.
//
// Returns a promise.
