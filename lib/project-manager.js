...
var ProjectManager = module.exports = function (options) {
    ...
    var optionsWithHeaders = _.cloneDeep(this.options);
    optionsWithHeaders['withHeaders'] = true;
    this.requestWithHeaders = request.bind(null, optionsWithHeaders, 'ProjectManager');
};
...
// Get header along with project
// returns {headers: ,response: }
ProjectManager.prototype.subscribeToProjectWithHeaders = function (accountName, projectName, session) {
  assert.string(accountName, 'accountName');
  assert.string(projectName, 'projectName');
  assert.string(session, 'session');

  return this.requestWithHeaders('subscribeToProject', {
    accountName: accountName,
    projectName: projectName
  }, {
    'session_id': session
