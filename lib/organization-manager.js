...
OrganizationManager.prototype.createTeam = function (name, info) {
    ...
    assert.optionalString(info.session, 'session_id');
    assert.optionalBool(info.isMigration, 'isMigration');
...
OrganizationManager.prototype.addMembers = function (data, session) {
    ...
    assert.optionalBool(data.skipEmailNotification, 'skipEmailNotification');
    assert.optionalBool(data.isMigration, 'isMigration');
...
OrganizationManager.prototype.memberOneOfGroup = function (organization, teamIds, memberId) {
    assert.string(organization, 'organization');
    assert.arrayOfString(teamIds, 'teamIds');
    assert.string(memberId, 'memberId');
