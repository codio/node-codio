    ...
    return function done(res) {
        ...
        if (result.code === 0) {
          ...
          if (result.response && result.response.code) {
            errorMessage = result.response.code;
          }
...
module.exports = function request(options, object, method, data, params, shouldSign) {
    ...
    var headers = ['Cookie:' + opts.cookie, 'Content-Type: application/json'];
    if (shouldSign) {
        var signature = sign(acrequest, options.provider, options.secretKey);
        headers.push('X-Codio-Sign-Timestamp: ' + signature.timestamp);
        headers.push('X-Codio-Sign: ' + signature.signature);
        headers.push('X-Codio-Provider: ' + options.provider);
    }

    return curl.request(opts.url + pathname, {
        headers: headers
    }, acrequest).then(createDone(pathname, object, method));
};
