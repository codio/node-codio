...
function sign(text, token) {
    var timestamp = new Date().getTime();
    var shasum = crypto.createHash('sha1');
    var signature = shasum.update(timestamp.toString() + text + token).digest('hex');
...
module.exports = function request(options, object, method, data, params, callback, shouldSign) {
    ...
    if (shouldSign) {
        var signature = sign(acrequest, options.remoteSecretKey);
    ...
    req.post({
        ...
        if (result.code === 0) {
            return callback(new VError('Got error from %s: %s', result.source, result.response.message));
        }
