...
module.exports = function request(options, object, method, data, params, shouldSign) {
    ...
    return req.postAsync({
        ...
        if (res.statusCode !== 200) {
            return Promise.reject(new VError('Got non 200 code: %d', res.statusCode));
        }

        var result = JSON.parse(body);

        if (result.code === 0) {
            return Promise.reject(new VError('Got error from %s: %s', result.source, result.response.message));
        }
