...
//
// Returns a promise.
ImportManager.prototype.importFromS3 = function (filePath, info, taskIdCallback) {
    ...
    assert.optionalString(info.stack, 'info.stack');
    assert.optionalObject(info.gigaBoxSlot, 'info.gigaBoxSlot');
    ...
    if (info.gigaBoxSlot) {
        data.gigaBoxSlot = info.gigaBoxSlot;
    }

    return this.request('importFromS3', data, {
        session_id: info.session
    })
    .then(function (data) {
        if (typeof taskIdCallback === 'function') {
            taskIdCallback(data.message);
        }
        return self.ping(data.message, info.session);
    });
};
...
ImportManager.prototype.restoreUnits = function (path, bucket, ids) {
    ...
    var self = this;
    return this.requestSigned('restoreUnits', data, {})
      .then(function (data) {
          return self.ping(data.message);
      });
};
