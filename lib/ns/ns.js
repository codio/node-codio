...
p.connect = function (session) {
    ...
    var self = this;
    var Socket = Primus.createSocket({transformer: 'sockjs', pathname: 'p',
        parser: {
...
p.reconnect = function () {
    ...
    _.each(this.subs, function (info, channelId) {
        if (info.version) {
            ...
            self.subscribe(channelId, undefined, 'FROM ' + version, true);
        } else {
            self.subscribe(channelId, undefined, info.params, true);
        }
    });
};
...
p.subscribe = function (channelId, callback, params, force) {
    var needSub = false;
    if (!this.subs[channelId]) {
        needSub = true;
        this.subs[channelId] = {params: params, callbacks: {}};
    }
    var id = this.guid();
