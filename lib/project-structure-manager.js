...
//
// Returns a promise.
ProjectStructureManager.prototype.getFile = function (path, info, withoutContent) {
    ...
    return this.request('getFile', {
        ...
        path: path,
        format: withoutContent ? 'empty' : 'plain'
    }, {
...
//
// Returns a promise.
ProjectStructureManager.prototype.createFile = function (path, info) {
    assert.string(path, 'path');
    assert.object(info, 'info');
    assert.string(info.user, 'info.user');
    assert.string(info.project, 'info.project');
...
// Move secure folder in/out project
//
// Returns a promise.
ProjectStructureManager.prototype.moveSecureFolder = function (projectId, direction) {
    assert.string(projectId, 'projectId');
    assert.bool(direction, 'direction');

    return this.requestSigned('moveSecureFolder', {
        projectId: projectId,
        direction: direction
    }, {});
};
