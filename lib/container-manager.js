...
//
// Returns a promise.
ContainerManager.prototype.runEphemeral = function (options) {
    assert.object(options, 'options');
    assert.object(options.run, 'options.command');
    assert.string(options.run.cmd, 'options.stack');

    var self = this;
    return this.requestSigned('runEphemeral', options, {})
    .then(function (data) {
        return self.ping(data.message);
    });
};
