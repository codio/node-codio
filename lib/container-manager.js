...
// Run command in container without session
//
// options - object {
//   container: container name
//   or
...
//
// Returns a promise.
ContainerManager.prototype.runEphemeral = function (options, replyParameters) {
    assert.object(replyParameters, 'replyParameters');
    assert.object(options, 'options');
    assert.arrayOfString(options.command, 'userIds');
    assert.string(options.stack, 'options.stack');

    var self = this;
    return this.requestSigned('runEphemeral', {
...
//
// Returns a promise.
ContainerManager.prototype.runInternal = function (options) {
    assert.object(options, 'options');
    assert.object(options.run, 'options.run');
    assert.string(options.run.cmd, 'options.run.cmd');

    var self = this;
    return this.requestSigned('run', options, {})
    .then(function (data) {
        return self.ping(data.message);
    });
};
